...
const Bookshelf: React.FC = () => {
  ...
  const syncWithServer = async () => {
    try {
      ...
      if (res.ok) {
        ...
        // Token might be invalid or expired. Clear it so we fallback to guest mode.
    ...
    const fetchIsbn = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `https://openlibrary.org/search.json?title=${encodeURIComponent(title)}&author=${encodeURIComponent(author)}&fields=isbn`,
        );
        const data = await res.json();
          ...
          if (bibkeys.length > 0) {
            await new Promise<void>((resolve) => {
              ...
              window[callbackName] = (gbObj: any) => {
                ...
                if (gbObj) {
                  const hasPreview = Object.values(gbObj).some(
                    (info: any) => info.embeddable,
                  );
      ...
      if (window.google?.books?.DefaultViewer) {
        ...
        if (container) {
          ...
          viewer.load(
            ...
            () => {
              container.innerHTML =
                '<div class="flex items-center justify-center p-8 text-slate-500 font-medium h-full">Preview not available for this book.</div>';
            },
            () => {
  ...
  const periods = volumesData.nodes.filter(
    (n: any) => n.type === "period",
  ) as any[];
  const works = volumesData.nodes.filter(
    (n: any) => n.type === "work",
  ) as Work[];
  const genres = volumesData.nodes.filter(
    (n: any) => n.type === "genre",
  ) as any[];
  const regions = volumesData.nodes.filter(
    (n: any) => n.type === "region",
  ) as any[];

  const getPeriodForWork = (workId: string) => {
    const genreEdge = volumesData.edges.find(
      (e: any) => e.to === workId && e.type === "includes",
    );
    if (!genreEdge) return null;
    const periodEdge = volumesData.edges.find(
      (e: any) => e.to === genreEdge.from && e.type === "contains",
    );
    return periodEdge ? periodEdge.from : null;
  };

  const getGenreForWork = (workId: string) => {
    const edge = volumesData.edges.find(
      (e: any) => e.to === workId && e.type === "includes",
    );
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-slate-200">
      <div className="max-w-6xl mx-auto px-6 py-20">
        <div className="flex justify-between items-center mb-16">
          <Link
            to="/"
            className="inline-flex items-center space-x-2 text-slate-400 hover:text-slate-950 font-bold tracking-tight transition-colors"
          >
        ...
        <div className="space-y-32">
          {periods.map((period) => {
            ...
            periodWorks.forEach((work) => {
              const genre =
                getGenreForWork(work.id) ||
                (work.genre ? genres.find((g) => g.id === work.genre) : null);
            ...
            return (
              <section key={period.id} className="animate-fade-in">
                ...
                <div className="space-y-16">
                  {Object.entries(worksByGenre).map(
                    ([genreLabel, genreWorks]) => (
                      <div
                        key={genreLabel}
                        className="bg-white rounded-3xl p-8 border border-slate-400 shadow-sm"
                      >
                          ...
                          <div className="md:col-span-1">
                            <div className="sticky top-24">
                              ...
                              <p className="text-xs text-slate-400 font-medium">
                                {genreWorks.length}{" "}
                                {genreWorks.length === 1 ? "Work" : "Works"}
                              </p>
                            </div>
                          </div>
            ...
            <div className="relative p-8 sm:p-12">
                ...
                <div className="flex flex-col gap-y-3">
                  ...
                  <div className="flex items-center space-x-3 text-slate-600 flex-wrap gap-y-2">
                    ...
                    <div className="flex items-center space-x-2 px-3 border-l border-slate-300">
                      ...
                      <div className="px-3 py-1 bg-slate-100 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                        {selectedWork.region
                          ? regions.find((r) => r.id === selectedWork.region)
                              ?.label
                          : "Unknown Region"}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center space-x-4 text-slate-600 flex-wrap gap-y-2">
                      ...
                      <div className="flex items-center space-x-4 px-3 border-l border-slate-300">
                        <a
                          ...
                          className="flex items-center space-x-1 text-sm font-semibold text-slate-500 hover:text-amber-600 transition-colors"
                        >
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center space-x-1 text-sm font-semibold text-slate-500 hover:text-amber-900 transition-colors"
                        >
              ...
              <div className="space-y-8">
                ...
                {selectedWork.author && (
                  <div className="pt-4 sm:pt-6">
                    ...
                    <p className="text-base text-slate-500 leading-relaxed">
                      {selectedWork.authorDescription ||
                ...
                <div className="pt-4 sm:pt-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <span
                      className={`text-[10px] font-bold ${
                        (userProgress[selectedWork.id]?.notes?.length || 0) >=
                        500
                          ? "text-rose-500"
                          : "text-slate-300"
                      }`}
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      updateNotes(selectedWork.id, e.target.value.slice(0, 512))
                    }
              ...
              <div className="mt-8 sm:mt-12 grid grid-cols-1 sm:grid-cols-3 gap-4 xl:gap-8">
                ...
                <div className="relative group w-full">
                  ...
                  <button
                    onClick={() => {
                      if (!isFetchingIsbn && previewAvailable)
                        setShowPreview(!showPreview);
                    }}
                    disabled={
                      isFetchingIsbn || (!isFetchingIsbn && !previewAvailable)
                    }
